...
<script type="text/javascript">
			function insert3rdpartycode()
			{
				...
				if (study_name) {
                    if (window.parent.document.getElementById("<?php echo $jform;?>study_name"))
				    {window.parent.document.getElementById("<?php echo $jform;?>study_name").value = study_name;}
				}
				if (study_alias) {
				    if (window.parent.document.getElementById("<?php echo $jform;?>study_alias"))
                    {window.parent.document.getElementById("<?php echo $jform;?>study_alias").value = study_alias;}
				}
				if (study_description) {
				    if (window.parent.document.getElementById("<?php echo $jform;?>study_description"))
                    {window.parent.document.getElementById("<?php echo $jform;?>study_description").value = study_description;}
				}
                if (tags) {
                    if (window.parent.document.getElementById("<?php echo $jform;?>tags"))
                    {window.parent.document.getElementById("<?php echo $jform;?>tags").value = tags;}
                }
				if (dur_hrs) {
				    if (window.parent.document.getElementById("<?php echo $jform;?>dur_hrs"))
                    {window.parent.document.getElementById("<?php echo $jform;?>dur_hrs").value = dur_hrs;}
				}
				if (dur_mins) {
				    if (window.parent.document.getElementById("<?php echo $jform;?>dur_mins"))
                    {window.parent.document.getElementById("<?php echo $jform;?>dur_mins").value = dur_mins;}
				}
				if (dur_secs) {
				    if (window.parent.document.getElementById("<?php echo $jform;?>dur_secs"))
                    {window.parent.document.getElementById("<?php echo $jform;?>dur_secs").value = dur_secs;}
				}
				if (video_link) {
                    if (window.parent.document.getElementById("<?php echo $jform;?>video_link"))
                    {window.parent.document.getElementById("<?php echo $jform;?>video_link").value = video_link;}
                    else
                    {window.parent.document.getElementById('extrainputs').innerHTML += '<input type="hidden" id="videooverride" name="videooverride" value ="'+video_link+'" />'}
                }
                if (video_type) {
                    if (window.parent.document.getElementById("<?php echo $jform;?>video_type"))
                    {window.parent.document.getElementById("<?php echo $jform;?>video_type").value = video_type;}
                    else
                    {window.parent.document.getElementById('extrainputs').innerHTML += '<input type="hidden" id="videotypeoverride" name="videotypeoverride" value ="'+video_type+'" />'}
                }
				if (imagelrg) {
                    if (window.parent.document.getElementById("<?php echo $jform;?>imagelrg"))
                    {window.parent.document.getElementById("<?php echo $jform;?>imagelrg").value = imagelrg;}
                    if (window.parent.document.getElementById("<?php echo $jform;?>image_folderlrg"))
		...
		<table class="admintable" width="100%" align="center">    
			...
			<tr>
				...
				<td>
					<textarea class="text_area" cols="10" rows="4" name="study_description" id="study_description" style="width: 90%;"><?php echo $this->row->description;?></textarea>
				</td>
			</tr>  
            <tr>
                <td class="key">
		...
		<button style="margin: 10px 0 5px 10px;" onclick="thirdpartyclose();"><?php echo JText::_( 'COM_PREACHIT_EDITOR_BUTTON_CLOSE' ); ?></button>
		</div>
<?php }
