...
class PreachitViewTeacherlist extends JView
{
...
$new_pathway_item = ucfirst(JText::_('COM_PREACHIT_VIEW_TEACHERLIST'));
...
foreach ($rows as $row)
{
	$teachers[$i] = PIHelperinfobuilder::teacherinfo($row, $params);
    
    //assign study text to use plugins
    //Process the prepare content plugins

    $teachers[$i]->text = $teachers[$i]->description;
    $limitstart = 0;
    $dispatcher    =& JDispatcher::getInstance();
    JPluginHelper::importPlugin('content');
    $results = $dispatcher->trigger('onContentPrepare', array ('com_preachit.article', & $teachers[$i], & $params, $limitstart));
    $teachers[$i]->description = $teachers[$i]->text;
    unset($teachers[$i]->text);
