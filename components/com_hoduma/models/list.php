...
class HodumaModelList extends JModel
{
	var $_total = null;
	...
	function _buildQuery()
	{
			...
			switch ($type)
			{
				...
				case 'assigned':
					...
					//Hoduma: I've moved the component parameters to the right place, so we get it much easier
					//$query = 'SELECT s.id AS sid FROM #__hoduma_config AS c JOIN #__hoduma_statuses AS s ON s.id=c.closestatus';
					//$db =& JFactory::getDBO();
					//$db->setQuery($query);
					//$closed = $db->loadRow();
					...
					$where = ' WHERE p.status <> '.$closed.' AND p.rep='.$hid[0];
					break;
				case 'all':
...
//OLD			$query = "SELECT p.id as id, p.title as title, p.uid as uid, p.start_date as start_date, pr.pname as priority, s.sname as status FROM #__hoduma_problems AS p LEFT OUTER JOIN #__hoduma_priority AS pr ON p.priority=pr.priority_id LEFT OUTER JOIN #__hoduma_status AS s ON p.status=s.id ";
//ADDS REPNAME		$query = "SELECT p.id as id, p.title as title, p.uid as uid, p.start_date as start_date, pr.pname as priority, s.sname as status, ju.username as repname FROM #__hoduma_problems AS p LEFT OUTER JOIN #__hoduma_priority AS pr ON p.priority=pr.priority_id LEFT OUTER JOIN #__hoduma_status AS s ON p.status=s.id LEFT OUTER JOIN #__hoduma_users AS u ON u.id = p.rep LEFT OUTER JOIN #__users AS ju ON ju.id = u.joomla_id";
			$query = "SELECT p.id as id, p.title as title, p.uid as uid, p.start_date as start_date, pr.pname as priority, s.sname as status, ju.username as repname, maxresults.maxdate AS maxdate, pr.fontcolor AS prioritycolor, s.fontcolor AS statuscolor FROM #__hoduma_problems AS p LEFT OUTER JOIN #__hoduma_priorities AS pr ON p.priority=pr.id LEFT OUTER JOIN #__hoduma_statuses AS s ON p.status=s.id LEFT OUTER JOIN #__hoduma_users AS u ON u.id = p.rep LEFT OUTER JOIN #__users AS ju ON ju.id = u.joomla_id LEFT OUTER JOIN (SELECT max(addDate) as maxdate, problem_id FROM #__hoduma_notes GROUP BY problem_id) maxresults ON p.id = maxresults.problem_id";
			$query = $query.$where;
			$query = $query.$orderby;
			return $query;
	}
