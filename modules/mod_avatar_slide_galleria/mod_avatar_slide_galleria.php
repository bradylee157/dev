...
if (!file_exists(JPATH_ROOT .DIRECTORY_SEPARATOR. 'plugins' .DIRECTORY_SEPARATOR.'content'.DIRECTORY_SEPARATOR. 'avatar_slide_galleria'.DIRECTORY_SEPARATOR.'assets'.DIRECTORY_SEPARATOR.'js'.DIRECTORY_SEPARATOR.'galleria-1.2.8.min.js')) {
	$document->addScript(JURI::base().'modules/mod_avatar_slide_galleria/assets/js/galleria-1.2.8.min.js');
} else { 	
	$document->addScript(JURI::base().'plugins/content/avatar_slide_galleria/assets/js/galleria-1.2.8.min.js');			
}
switch ($source) {
	case 'folder':
...
{ 
		...
		{	
			...
			Galleria.on('loadfinish', function(){
				...
				$(this._dom.container).css('max-height', '<?php echo $sliderHeight;?>');
			});
