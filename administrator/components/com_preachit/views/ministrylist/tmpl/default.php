...
$link = JFilterOutput::ampReplace( 'index.php?option=' . $option . '&controller=ministrylist&task=edit&cid[]='. $row->id );

//determine name column
if ($user->authorise('core.edit', 'com_preachit')) 
{
$name = '<a href="'.$link.'">'.$row->ministry_name.'</a>';}
else {$name = $row->ministry_name;}
...
<td>
<?php echo JString::substr($row->ministry_description, 0, 149); ?>
</td>
<td class="center">
<?php if ($row->language=='*'):?>
...
<input type="hidden" name="controller" value="ministrylist" />
