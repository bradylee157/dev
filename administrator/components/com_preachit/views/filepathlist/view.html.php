...
class PreachitViewFilepathlist extends JView
{
...
//state list
		...
		$filter_state = $app->getUserStateFromRequest( $option.'filter_statefp', 'filter_statefp'. '', 'int' );
$this->assignRef('filter_state',	$filter_state);
$selectstate = Tewebdetails::stateselector();
$this->assignRef('state_list', JHTML::_('select.genericList', $selectstate, 'filter_statefp', 'class="inputbox" size="1" onchange="this.form.submit()"', 'value', 'text', $filter_state ));
...
JToolBarHelper::title( JText::_( 'COM_PREACHIT_ADMIN_TITLE_FOLDERS' ), 'folder.png');
