...
//state list
		...
		$filter_state = $app->getUserStateFromRequest( $option.'filter_statepod', 'filter_statepod'. '', 'int' );
$this->assignRef('filter_state',	$filter_state);
$selectstate = Tewebdetails::stateselector();
$this->assignRef('state_list', JHTML::_('select.genericList', $selectstate, 'filter_statepod', 'class="inputbox" size="1" onchange="this.form.submit()"', 'value', 'text', $filter_state ));
$this->addToolbar();
parent::display($tpl);
}
...
JToolBarHelper::title( JText::_( 'COM_PREACHIT_ADMIN_TITLE_PODCASTS' ), 'podcast.png');
