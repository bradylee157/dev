...
<th width="14%"><?php echo JText::_('COM_PREACHIT_ADMIN_ROLELIST');?></th>
...
$link = JFilterOutput::ampReplace( 'index.php?option=' . $option . '&controller=teacherlist&task=edit&cid[]='. $row->id );
...
{$name = '<a href="'.$link.'">'.$row->teacher_name.' '.$row->lastname.' '.JText::_('COM_PREACHIT_ADMIN_RECORD_CHECKED_OUT').'</a>';}
elseif ($row->checked_out && $this->checkin == 1)
{$name = $row->teacher_name.' '.$row->lastname.' '.JText::_('COM_PREACHIT_ADMIN_RECORD_CHECKED_OUT');}
