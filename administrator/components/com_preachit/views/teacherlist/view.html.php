...
$filter_state = $app->getUserStateFromRequest( $option.'filter_statete', 'filter_statete'. '', 'int' );
...
$this->assignRef('state_list', JHTML::_('select.genericList', $selectstate, 'filter_statete', 'class="inputbox" size="1" onchange="this.form.submit()"', 'value', 'text', $filter_state ));
...
JToolBarHelper::title( JText::_( 'COM_PREACHIT_ADMIN_TITLE_TEACHERS' ), 'teacher.png');
...
if ($this->filter_state == -2 && $user->authorise('core.delete', 'com_preachit')) {
            JToolBarHelper::deleteList('', 'remove','TE_TOOLBAR');
