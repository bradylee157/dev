...
class PreachitModelPodcastedit extends JModelForm
{
        ...
        public function getForm($data = array(), $loadData = true) 
        {
                ...
                $form = JForm::getInstance('com_preachit.podcastedit', 'podcastedit', array('control' => 'jform', 'load_data' => $loadData));
                return $form;
        }
	...
	public function &getData() 
        {
                ...
                if (empty($this->data)) 
                {
                        ...
                        if (empty($data)) 
                        {
                                ...
                                $query->select('*');
                                $query->from('`#__pipodcast`');
                                $query->where('id = ' . (int)$selected[0]);
                                $db->setQuery((string)$query);
                                $data = & $db->loadAssoc();
                                if (isset($data['teacher_list']))
                                {$data['teacher_list'] = explode(',', $data['teacher_list']);}
                                if (isset($data['ministry_list']))
                                {$data['ministry_list'] = explode(',', $data['ministry_list']);}
                                if (isset($data['series_list']))
                                {$data['series_list'] = explode(',', $data['series_list']);}
                                if (isset($data['media_list']))
                                {$data['media_list'] = explode(',', $data['media_list']);}
                        }
								 $this->data = $data;						
                }
                return $this->data;
