...
class plgContentScriptureLinks extends JPlugin
{
	...
	public function onContentPrepare($context, &$row, &$params, $page = 0)
  {
    ...
    $row->text = $this->utf8dec($row->text);
    if($replace_all){
      $books = $this->getBooksRegex($bible_version);
      $st = "/(?=\S)(\{bible\s\d{1,2}\})?\b(".$books.")(\s)(\d{1,3})([:,](?=\d))?(\d{1,3})?[-]?(\d{1,3})?([,](?=\d))?(\d{1,3})?([:](?=\d))?(\d{1,3})?[-]?(\d{1,3})?(\{\/bible\})?/iu";
      $row->text = preg_replace_callback( $st, array( &$this, 'link_insert'), $row->text );
  ...
  function link_insert(&$matches)
  {
    ...
    if($use_gb){
      ...
      $alinkattribs['title'] = JText::_('AUDIO').' {STRING}';
    } else {
      $document	= JFactory::getDocument();
      $document->addCustomTag('<script type="text/javascript">');
      $document->addCustomTag('    var WStyle1 = "width=800,height=500,scrollbars=1";');
    ...
    $alink = preg_replace_callback( $st, array( &$this, 'split_verse'), str_replace(".","",$scripture) );
    $alink = explode(",",$alink); 
    $scripture_link = "";
    $chklang = "";
    $biblebooks = $this->getBooks($bible_version);
    if($use_tooltip){
      ...
      $slinktext = JText::_('READVERSE')." ".$ucslinkstrg;
      $linkattribs['title'] = $slinktext;
      $slink = '<center>'.JHTML::_('link', JRoute::_('http://classic.biblegateway.com/passage/index.php?search='.urlencode($ucslinkstrg).';&version='.$bible_version.';&interface=print'), $ucslinkstrg, $linkattribs).'&nbsp;{CSTRING}{ASTRING}</center>';
      $chklang = $this->getLang($bible_version);
      if($com_link && $chklang == 'EN'){
        $clink = preg_replace_callback( $st, array(&$this, 'getverse_com'), $scripture );
        $clink = explode(",",$clink); 
      ...
      if($audio_link){
        ...
        $audlink = ' '.JHTML::_('link', JRoute::_('http://classic.biblegateway.com/resources/audio/flash_play.php?source='.$source.'&aid='.$aid.'&book='.rtrim($alink[2]).'&chapter='.rtrim($alink[0],":").'&end_chapter='.rtrim($alink[1],":")), '<img style="width:16px; height:9px;" src="http://www.biblegateway.com/resources/audio/images/sound.gif" border="0" />', $alinkattribs);
      ...
      $scripture_link = JHTML::_('link', JRoute::_('http://classic.biblegateway.com/passage/index.php?search={QUERY};&version='.$bible_version.';&interface=print'), '{STRING}', $linkattribs);
      $scripture_link = str_replace('{STRING}', !$this->checkutf8($bible_version) ? htmlentities(trim($scripture)) : trim($scripture), $scripture_link);
      $scripture_link = str_replace('{QUERY}', !$this->checkutf8($bible_version) ? htmlentities(str_replace('&nbsp;',' ',urlencode($this->customCharHandling($ucslinkstrg,$bible_version)))) : str_replace('&nbsp;',' ',urlencode($this->customCharHandling($ucslinkstrg,$bible_version))), $scripture_link);
    } else {
      if($use_gb){
        $linkattribs['onclick'] = '';
        $linkattribs['class'] = 'modal';
      ...
      $scripture_link = str_replace('{QUERY}', !$this->checkutf8($bible_version) ? htmlentities(str_replace('&nbsp;',' ',urlencode($this->customCharHandling($ucslinkstrg,$bible_version)))) : str_replace('&nbsp;',' ',urlencode($this->customCharHandling($ucslinkstrg,$bible_version))), $scripture_link);
      if($audio_link){
        $scripture_link .= ' '.JHTML::_('link', JRoute::_('http://classic.biblegateway.com/resources/audio/flash_play.php?source='.$source.'&aid='.$aid.'&book='.rtrim($alink[2]).'&chapter='.rtrim($alink[0],":").'&end_chapter='.rtrim($alink[1],":")), '<img style="width:16px; height:9px;" src="http://www.biblegateway.com/resources/audio/images/sound.gif" border="0" />', $alinkattribs);
        $alink[0] == $alink[1] ? $averse = $alink[0] : $averse = $alink[0].'-'.$alink[1];
        if(!empty($alink[2])) {$ucslinkstrg = ucwords($biblebooks[$alink[2]]['book']).' '.$smatch[3];}else{$ucslinkstrg = $alink[3]." ".$averse;}
      ...
      $chklang = $this->getLang($bible_version);
      if($com_link && $chklang == 'EN'){
        $clink = preg_replace_callback( $st, array(&$this, 'getverse_com'), str_replace(".","",$scripture) );
        $clink = explode(",",$clink); 
        $clink[1] == 'index' ? $clink_chap = '' : $clink_chap = ' '.$clink[1];
        $clinkstrg = preg_replace('/(\d+)\s?(\w+)/i', '${1} $2', $clink[0]);
        $key = $this->sl_array_search_recursive($clinkstrg, $biblebooks, true, $bible_version);
        preg_match('/(\d)?\s?(\w+)\.?\s?([\d\.\:\,\-]+)?/i', $scripture, $smatch);
        $linkattribs['title'] = JText::_('COMMENTARY').' '.ucwords($biblebooks[$key[0]]['book']).' '.$smatch[3];
        if(!empty($key[0])) {$ucclinkstrg = str_replace(" ","",$biblebooks[$key[0]]['book']);}else{$ucclinkstrg = str_replace(" ","",$clinkstrg);}
  ...
  function sl_array_search_recursive( $needle, $haystack, $type=false, $bible_version )
  {
     $path = NULL;
     ...
     while (!$path && (list($toss,$k)=each($keys))) {
        ...
        if (is_scalar($v)) {
          ...
          if(!$this->checkutf8($bible_version))  $v = htmlentities($v);
  ...
  function getBooksRegex($bible_version){
    $langfile = strtolower($this->getLang($bible_version));
    ...
    $this->checkutf8($bible_version) ? $bookstr = $books : $bookstr = htmlentities($books);
